...
function writeAt(buffer: string[], col: number, value: string): void {
  for (const [index, char] of [...value].entries()) {
    buffer[col + index] = char;
  }
}
...
function stringifyPlayerLine(
  ...
  const buffer: string[] = Array.from({ length: COL_RANK + 5 }, () => ' ');
  ...
  if (player.federation !== undefined) {
    ...
    writeAt(
      buffer,
      COL_FEDERATION,
      pad(player.federation.slice(0, 3), 3, 'left'),
    );
  }
  ...
  if (player.fideId !== undefined) {
    ...
    writeAt(buffer, COL_FIDE_ID, pad(player.fideId.slice(0, 12), 12, 'left'));
  }
  ...
  writeAt(buffer, COL_POINTS, pad(pointsString, 4, 'right'));
  ...
  if (results.length > 0) {
    for (const result of results) {
      ...
      while (buffer.length < slot + ROUND_ENTRY_LENGTH) {
        buffer.push(' ');
      }
      ...
      writeAt(buffer, slot, entry);
    }
    // Trim trailing spaces only up to the rounds section; keep round entries intact
    const header = buffer.slice(0, ROUND_RESULTS_OFFSET).join('').trimEnd();
    const rounds = buffer.slice(ROUND_RESULTS_OFFSET).join('');
...
export default function stringify(
  ...
  if (meta?.roundDates !== undefined && meta.roundDates.length > 0) {
    ...
    for (const [index, date] of meta.roundDates.entries()) {
      ...
      while (buffer.length < pos + ROUND_ENTRY_LENGTH) {
        buffer.push(' ');
      }
      writeAt(buffer, pos, date.slice(0, 8));
    }
  ...
  if (version === 'TRF26') {
    for (const player of data.players) {
      ...
      for (const nrs of nationalRatings) {
        ...
        writeAt(buffer, 0, nrs.federation.slice(0, 3));
        ...
        if (player.name !== undefined) {
          writeAt(buffer, COL_NAME, pad(player.name.slice(0, 33), 33, 'left'));
        }
        writeAt(buffer, COL_RATING, pad(String(nrs.rating), 4, 'right'));
  ...
  if (version === 'TRF26') {
    ...
    for (const team of teams) {
      const buffer: string[] = Array.from({ length: 72 }, () => ' ');
      buffer[0] = '3';
      buffer[1] = '1';
      buffer[2] = '0';
      const teamIdString = String(Number(team.id));
      writeAt(buffer, 4, pad(teamIdString, 3, 'right'));
      writeAt(buffer, 8, pad(team.name.slice(0, 32), 32, 'left'));
      if (team.nickname !== undefined) {
        writeAt(buffer, 41, pad(team.nickname.slice(0, 5), 5, 'left'));
      }
      writeAt(buffer, 54, pad(team.matchPoints.toFixed(1), 6, 'right'));
      writeAt(buffer, 61, pad(team.gamePoints.toFixed(1), 6, 'right'));
      writeAt(buffer, 68, pad(String(team.rank), 3, 'right'));
      for (const [index, id] of team.playerIds.entries()) {
        ...
        writeAt(buffer, pos, pad(idString, 4, 'right'));
      }
      lines.push(buffer.join('').trimEnd());
    }
  }
  ...
  if (version === 'TRF26') {
    ...
    for (const record of teamRoundResults) {
      if (record.tag === '801') {
        const buffer801: string[] = Array.from({ length: 22 }, () => ' ');
        writeAt(buffer801, 0, '801');
        const teamIdString801 = String(Number(record.teamId));
        writeAt(buffer801, 3, pad(teamIdString801, 4, 'right'));
        if (record.nickname !== undefined) {
          writeAt(buffer801, 7, pad(record.nickname.slice(0, 5), 5, 'left'));
        }
        writeAt(buffer801, 12, pad(String(record.matchPoints), 4, 'right'));
        writeAt(buffer801, 16, pad(record.gamePoints.toFixed(1), 6, 'right'));
        for (const r of record.results as TeamRoundResult801[]) {
          const pos = 22 + (r.round - 1) * 16;
          while (buffer801.length < pos + 16) {
            buffer801.push(' ');
          }
          if (r.type === undefined) {
            const oppString =
              r.opponentId === null ? '' : String(Number(r.opponentId));
            writeAt(buffer801, pos, `  ${pad(oppString, 3, 'right')} ${r.raw}`);
          } else {
            ...
            writeAt(buffer801, pos + 5, `  ${marker}       `);
          }
        }
        lines.push(buffer801.join('').trimEnd());
      } else {
        // 802
        const buffer802: string[] = Array.from({ length: 28 }, () => ' ');
        writeAt(buffer802, 0, '802');
        const teamIdString802 = String(Number(record.teamId));
        writeAt(buffer802, 4, pad(teamIdString802, 3, 'right'));
        if (record.nickname !== undefined) {
          writeAt(buffer802, 8, pad(record.nickname.slice(0, 5), 5, 'left'));
        }
        writeAt(buffer802, 14, pad(record.matchPoints.toFixed(1), 6, 'right'));
        writeAt(buffer802, 21, pad(record.gamePoints.toFixed(1), 6, 'right'));
        for (const r of record.results as TeamRoundResult802[]) {
          const pos = 28 + (r.round - 1) * 13;
          while (buffer802.length < pos + 13) {
            buffer802.push(' ');
          }
          if (r.type === undefined) {
            const oppString =
              r.opponentId === null ? '' : String(Number(r.opponentId));
            writeAt(buffer802, pos, pad(oppString, 3, 'right'));
            if (r.color !== undefined) {
              buffer802[pos + 4] = r.color;
            }
            writeAt(
              buffer802,
              pos + 6,
              pad(r.gamePoints.toFixed(1), 4, 'right'),
            );
            if (r.forfeit === true) {
              buffer802[pos + 10] = 'f';
            }
        ...
        lines.push(buffer802.join('').trimEnd());
      }
    }
  }
